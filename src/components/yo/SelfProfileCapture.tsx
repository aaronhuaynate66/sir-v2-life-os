...
import { extractSelfProfileImage, HttpError } from '@/lib/identity/captureClient'
...
export function SelfProfileCapture({ onClose }: SelfProfileCaptureProps) {
  ...
  const scan = useCallback(async () => {
    ...
    setUsedCount(results.length)

    // Nada legible/aprovechable: ni aportó campos nuevos ni hay texto.
    const nothingUsable =
      !proposal.hasChanges &&
  ...
  const save = useCallback(() => {
    ...
    toast.success('Identidad actualizada', {
      description: usedCount > 1 ? `Combiné ${usedCount} pantallazos.` : 'Desde tu pantallazo.',
    })
  }, [draft, setIdentityProfile, usedCount])
      ...
      <div className="space-y-3">
        <div className="rounded-md border border-warn/30 bg-warn/5 p-3 text-xs flex items-start gap-2">
          ...
          <span className="text-warn-foreground">
            ...
            las <span className="font-medium">secciones del perfil</span> (no la página entera), con
            la letra grande. No guardé nada.
          </span>
        </div>
  ...
  // ─── IDLE / WORKING (selección de imágenes) ───────────────────────
  return (
    <div className="space-y-3 rounded-md border border-border/60 bg-muted/10 p-3">
      <div className="flex items-center gap-2">
        <Camera size={14} strokeWidth={1.75} className="text-muted-foreground/70" aria-hidden="true" />
        <span className="text-xs font-medium text-foreground">Completar con pantallazos</span>
      </div>
      <p className="text-[11px] text-muted-foreground/80 leading-relaxed">
        Subí <span className="font-medium text-foreground">capturas de TU propio LinkedIn y/o Instagram</span>{' '}
        (varias secciones). Extraigo tus roles, intereses, ubicación y trayectoria y te lo muestro para
        revisar — <span className="font-medium text-foreground">nunca piso lo que ya escribiste</span>.
      </p>
...
function ProposalReview({
  ...
  return (
    <div className="space-y-4 rounded-md border border-primary/20 bg-muted/10 p-3">
      ...
      <p className="text-[11px] text-muted-foreground/80 leading-relaxed">
        {usedCount > 1 ? `Combiné ${usedCount} pantallazos. ` : ''}
        {added > 0
