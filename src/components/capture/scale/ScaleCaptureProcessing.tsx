'use client'
// SIR V2 — Step 2: loading mientras Vision procesa.

import { Loader2 } from 'lucide-react'
import { Card, CardContent } from '@/components/ui/card'

interface ScaleCaptureProcessingProps {
  /** URL local (createObjectURL) del blob comprimido para mostrar thumbnail */
  previewUrl: string
}

export function ScaleCaptureProcessing({ previewUrl }: ScaleCaptureProcessingProps) {
  return (
      ...
      <CardContent className="p-4 sm:p-6 flex flex-col sm:flex-row items-center gap-6">
        ...
        <div className="flex-1 flex flex-col items-center sm:items-start gap-2 text-center sm:text-left">
          ...
          <p className="text-xs text-muted-foreground max-w-md">
            Estamos extrayendo las métricas de la imagen. Esto toma 3–8 segundos.
            Si la imagen es nítida, Claude Vision reconoce peso, IMC, grasa
            corporal, masa muscular y el resto del panel.
          </p>
        </div>
      </CardContent>
    </Card>
