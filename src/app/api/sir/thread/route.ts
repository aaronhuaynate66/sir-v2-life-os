...
//
// Response 200: { turns: [{ role:'user'|'sir', text }] }  (orden cronológico)

import { NextResponse } from 'next/server'

import { createClient } from '@/lib/supabase/server'
import { getSirThread } from '@/lib/sir/thread'
...
export async function GET() {
  ...
  const turns = await getSirThread(supabase, authData.user.id, 40)
