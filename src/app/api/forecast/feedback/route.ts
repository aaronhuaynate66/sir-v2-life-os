...
export async function POST(req: NextRequest) {
  ...
  try {
    await supabase.from('forecast_feedback').insert({
    ...
    return NextResponse.json({ ok: true, label, recalibration: recal, anchored: !!(eventDate && (categories.includes('periodo') || categories.includes('pms'))) })
  } catch (e) {
