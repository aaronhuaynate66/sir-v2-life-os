...
export default function DashboardPage() {
  ...
  function handleAddFinance() { const amt = parseFloat(finAmount); if (isNaN(amt) || amt <= 0) return; addFinancialMovement({ id: `f_${Date.now()}`, type: 'expense', amount: amt, currency: 'USD', category: 'other', description: finDesc || 'Movimiento rapido', date: new Date().toISOString().split('T')[0], recurrent: false, tags: [] }); setFinAmount(''); setFinDesc('') }
      ...
      <div className="max-w-5xl mx-auto px-6 py-8">
        ...
        <div className="grid grid-cols-12 gap-4">
          <div className="col-span-12 md:col-span-4 space-y-4">
            ...
            <Card><Label>Finanzas</Label><Row label="Estabilidad" value={`${fin.stability.toFixed(1)}/10`} status={fin.riskLevel === 'low' ? 'ok' : fin.riskLevel === 'medium' ? 'warn' : 'bad'} /><Row label="Balance mensual" value={`$${fin.monthlyBalance.toLocaleString()}`} status={fin.monthlyBalance >= 0 ? 'ok' : 'bad'} /><Row label="Tasa ahorro" value={`${fin.savingsRate.toFixed(0)}%`} status={fin.savingsRate >= 20 ? 'ok' : fin.savingsRate >= 10 ? 'warn' : 'bad'} />{finAlerts[0] && <div className="mt-3 pt-3 border-t border-[#1a1a1a] text-[11px] text-[#f59e0b]">! {finAlerts[0].message}</div>}</Card>
          </div>
