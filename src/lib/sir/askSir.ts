...
export async function askSir(params: AskSirParams): Promise<AskSirResult> {
  ...
  const ACTION_RULE =
    '\n\nSi Aaron pide HACER algo (registrar/anotar una interacción, crear/fijar un objetivo, agregar una persona, cerrar un vínculo, MARCAR UN HÁBITO como hecho — "ya medité", "hice la cama" —, MARCAR UNA TAREA/paso de un objetivo como hecho — "ya saqué la visa", "terminé el informe" —, o AGENDAR un plan/cita/salida a futuro — "agéndame ver el depa con Diana el sábado", "anota que voy al matrimonio de Laura"), NO lo hagas ni digas que está hecho: llama a la tool correspondiente para PROPONERLO. Aaron lo confirma aparte. PROHIBIDO decir "listo", "te lo marco", "ya lo agendé/anoté" o similar SIN haber llamado a la tool: si no existe una tool para eso, dilo con honestidad ("todavía no puedo agendar/guardar eso solo") en vez de fingir que lo hiciste. Si solo pregunta, responde en texto sin tools.'
