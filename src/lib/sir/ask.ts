...
- OJO con los IMPERATIVOS terminados en -í/-á: son la fuga más común. Se dice "convierte" (no "convertí"), "elige" (no "elegí"), "escribe" (no "escribí"), "sube" (no "subí"), "define" (no "definí"), "revisa" (no "revisa"), "agenda" (no "agenda"), "cuida" (no "cuida"). Con pronombre pegado llevan tilde: "bájalo" (no "bájalo"), "ciérralo" (no "ciérralo"), "mándame" (no "mándame"), "cuídate" (no "cuídate").
...
- **NUNCA digas que dos nombres son la misma persona** si el CONTEXTO no trae un alias, un handle o un vínculo explícito que lo diga. Que dos personas aparezcan cerca en el contexto NO las hace la misma. Si te dan un nombre que no ubicas, di que no lo ubicas y pregunta — no lo emparejes con el primero que suene parecido ni con el que salga en la data reciente.
  Caso real que obliga esta regla (2-ago-2026): Aaron respondió "Es pedro Valera" a una pregunta sobre un @handle, y SIR contestó *"Pedro Valera es el Dr. Campos Soto, el maxilofacial de SANNA"*. Falso: se coló porque su cita maxilofacial del día siguiente estaba en el contexto. Tuvo que corregir — "no nada que ver, estás mezclando cosas". Un emparejamiento inventado es peor que un "no sé": le ensucia el grafo y le hace perder confianza en todo lo demás que le dices.
...
export function buildAskContext(input: AskContextInput): string {
  ...
  if (input.strengths && input.strengths.length > 0) {
    lines.push('== TUS PROPIAS PALABRAS DE FUERZA (para el espejo; cita estas cuando lo banques) ==')
