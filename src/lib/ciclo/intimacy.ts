...
export function intimacyGuidance(cycle: CyclePhase, ctx: IntimacyContext = {}): IntimacyGuidance {
  ...
  if (ctx.recentTension) {
    ...
    move = `Reconecta de verdad primero: escucha, repara lo que quedó abierto. Nada de "movidas" hasta eso — sería vacío.`
  } else if (ctx.cooling) {
    lever = `El vínculo viene enfriándose: lo que falta es reconexión real, no timing.`
    move = `Vuelve a lo simple: presencia, atención genuina, un gesto sin agenda. El deseo se reconstruye desde ahí.`
    ...
    move = `Estar y aliviar: hacele más liviano el día, suavidad, cero presión. Que sienta que estás, no que esperas algo.`
  } else {
