...
- **E3 Comportamiento:** 🟢 **~90% (cerrada en lo que rinde sin esperar datos).** 4/5: hábito-a-retomar y deuda-de-sueño al push de la mañana, quick-log de ánimo/energía de 1 tap en el panel (→ self_metrics), y retrospectiva MENSUAL (/resumen). Pendiente único: #4 (la correlación cierra en sugerencia conductual) — diferido a propósito hasta tener semanas de datos. Estado previo: Hábitos completos (CRUD/checkin/racha/nudge + `/habitos`), retrospectiva semanal (`/resumen`), motor biológico (recovery/deuda de sueño). **Cerrado esta sesión:** el loop salud/hábitos→día — el push de la mañana ahora trae *hábito a retomar* (racha rota) y *deuda de sueño* (de Apple Health). **Pendiente para cerrar E3:** (#2) quick-log de estado (1 tap, genera los datos subjetivos que faltan), (#4) que la correlación cierre en una sugerencia conductual no-clínica, (#5) retrospectiva mensual.
...
- **E5 Dirección de vida:** 🟢 **en marcha (artefacto real).** "Tu rumbo" (/yo) integra las 3 dimensiones: PASADO (hilo de objetivos + quiebres de vínculos en el tiempo), HACIA DÓNDE (tu ancla del año como 'norte'), y QUIÉN SOS (la reflexión IA enmarca por roles/bio). Determinístico + reflexión on-demand, anti-invención.
- **E6 AI-Native Human OS:** ⬜ visión norte.

**Red profesional COMPLETA (1→2→3 + 3b):** organización estructurada → nodo-empresa hub en el grafo → fichas /empresas/[slug] doble nivel → lectura estratégica IA on-demand en la ficha (línea estrategia-sí/engaño-no). 
