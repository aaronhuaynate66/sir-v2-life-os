...
>
> **Última actualización:** 26/07/2026 (reconciliación automática — ver bloque abajo).
...
## 🔁 RECONCILIACIÓN AUTOMÁTICA 2026-07-26

Pase de mantenimiento contra el código real (agente automático). ~40 commits desde el pase 07-24 (#943–#986: catálogo de seguidos IG, cruce médico-deportivo, ciclo del círculo en el brief, hybrid search Ola 3, barridos de voseo, endurecimiento del harness de eval). Los ítems de "PENDIENTES ACTUALES" (arriba) se re-chequearon contra este rango — **sin drift nuevo ahí**: voseo sigue con fixes activos pero sin remedir/cerrar (bug real, no resuelto); Ola 2 slice 4 sigue sin construir (`sortLearnings` en `src/lib/learnings/recall.ts:48` sigue con `preference` en rank 2, o sea 3ra posición, tal como dice el ítem); golden-set del harness sigue esperando decisión de Aaron (el flag `--from-feedback` y `eval/golden.jsonl` con 8 casos YA existen, coincide con la descripción del ítem); los 4 bloqueados por data/otra PC (cara capa 2, LinkedIn otra PC, Teams, DMs IG) sin cambios de código relevantes en el rango.

El drift real encontrado fue más viejo, arrastrado sin corregir por varios pases anteriores:

- ✅ **Inferencia LLM de dominio para objetivos de texto libre** — ya estaba HECHO desde el commit `07c9136` (PR #859, "feat(objetivos): inferir el dominio del objetivo dictado", **20/07/2026 23:42**) — anterior incluso al pase de reconciliación 07-20 de más abajo (que igual lo dio por pendiente ese mismo día, probablemente por corrida antes del merge) y no detectado por los pases 07-21/07-24 que vinieron después. Evidencia: `src/lib/objectives/smartPrompt.ts` — `ProposedSmart` suma `category?: GoalCategory` (línea 59); el prompt le pide al modelo inferir el dominio del texto (línea 70: "infiere el dominio del objetivo desde el texto..."); `parseSmart` lo valida contra el enum case-insensitive (líneas 136-138, con test); `SmartWizard.tsx` lo aplica al guardar en modo DICTADO (review-before-save; no pisa el dominio ya elegido en modo normal). Corregido in situ en las dos menciones de más abajo (reconciliaciones 07-20 y 07-21) y en la nota de la sección 06-08 — texto histórico intacto.
...
- ~~**Inferencia LLM de dominio para objetivos de texto libre** — sigue igual: `category` en `smartPrompt.ts`/`/api/objectives/smart` sigue siendo un INPUT, no algo inferido del párrafo libre.~~ ✅ **corregido 2026-07-26:** esta afirmación era incorrecta ya en esta misma fecha — el PR #859 (`07c9136`, 20/07/2026) agregó `category?: GoalCategory` inferido a `ProposedSmart` en `smartPrompt.ts`, aplicado en modo dictado por `SmartWizard.tsx`. Ver reconciliación 2026-07-26 arriba.
