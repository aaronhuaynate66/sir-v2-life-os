...
>
> **Última actualización:** 22/07/2026 (reconciliación automática — ver bloque abajo).
...
## 🔁 RECONCILIACIÓN AUTOMÁTICA 2026-07-22

Pase de mantenimiento contra el código real (agente automático). Volumen alto de commits desde el pase 07-21 (~35, sesión larga del 21/07 completa: gates de decisión, "cerebro"/feedback loop de SIR, bandeja "¿quién es quién?" por Telegram, canal conversacional unificado web↔Telegram, fixes de densidad UI/i18n, brief de la semana montado). La mayoría son features nuevas no listadas en este doc (se trackean por PR/issue, no por el backlog) — fuera de alcance reconciliar features nuevas, solo ítems que este doc ya lista como pendientes.

Un ítem SÍ tenía drift real, y grave: estaba mal marcado como pendiente en **3 lugares distintos** del doc, pese a que el código que lo resuelve es *anterior* al último pase (07-21) que lo re-confirmó como pendiente — o sea, el pase pasado no lo encontró, no que se haya hecho después:

- ✅ **Inferencia LLM de dominio para objetivos de texto libre** — ya HECHO. Commit `07c9136` (20/07/2026 23:42, "#10"): `src/lib/objectives/smartPrompt.ts` — `ProposedSmart.category?: GoalCategory` (línea 59), el prompt pide inferir el dominio del texto libre (línea 70), `parseSmart` valida contra el enum sin pisar el dominio elegido en modo normal (líneas 136-138). Wired en `SmartWizard.tsx` (modo dictado, líneas 168/204). 4 tests en `smartPrompt.test.ts` (líneas 89-96). Corregidas las 3 menciones: bloque 07-21 (arriba), bloque 07-20 (abajo) y la entrada original en "Etapa 4 follow-ups" (más abajo, sección EN PRODUCCIÓN 06-08), dejando el texto histórico con `~~strikethrough~~` y la nota ✅.

**Verificado y quedó IGUAL (sin drift), re-chequeado este pase:**
- **Auto-import desde calendario (Clay #6)** — sigue sin rastro en código (`grep` sin resultados).
- **Etapa 6 (AI-Native Human OS)** — `STRATEGIC_ROADMAP.md` línea 33 sigue en "⬜ visión norte".
- **Gantt fix del MASTER_PLAN** — sigue no verificable en este repo (sin componente "Gantt" en `src/`).
- **Split-brain / last-write-wins por fila** — sigue igual: `engine.ts` línea 296 sigue haciendo `upsert(..., { onConflict: 'id' })`.

**No re-verificado este pase** (fuera de alcance): todo lo demás, incluyendo lo ya marcado ✅/🟡 con evidencia en pases anteriores, y las ~35 features nuevas de la sesión 07-21 (no son ítems que este doc listara como "pendiente").
...
**Verificado y quedó IGUAL (sin drift), re-chequeado este pase:**
- ~~**Inferencia LLM de dominio para objetivos de texto libre** — sigue igual: `category` en `smartPrompt.ts`/`/api/objectives/smart` sigue siendo un INPUT, no algo inferido del párrafo libre.~~ ✅ **corregido 2026-07-22: en realidad YA estaba hecho incluso antes de este pase** (commit `07c9136`, 20/07/2026 23:42, o sea ~20 min antes de que este mismo pase 07-21 corriera) — el pase no lo detectó. Ver corrección con evidencia completa más abajo.
...
- **Etapa 4 follow-ups:** ~~Human OKRs estructurados~~ ✅ **HECHO** (migraciones `0040_objective_steps.sql`/`0041_objective_steps_okr.sql`: modelo KR→tarea de 2 niveles, `ObjectiveSteps.tsx`); ~~delta de relationship score (necesita snapshots históricos)~~ ✅ **HECHO** (`/api/person-score/snapshot` + cron `score-snapshots` + `BondEvolutionPanel.tsx`/`src/lib/people/bondEvolution.ts`); ~~tono de interacción desde `person_logs` en el engine~~ ✅ **HECHO** (`src/engines/alignment/index.ts` y `src/engines/relational-flags/index.ts` ya leen `person_logs`). Verificado 2026-07-17. ~~**Siguen pendientes de verdad:** inferencia LLM de dominio para objetivos de texto libre (hay inferencia SMART vía `SmartAssist.tsx`/`/api/objectives/smart`, pero no confirmé que infiera el DOMINIO desde texto libre — marcar "revisar" antes de darlo por hecho; re-verificado 2026-07-20, `smartPrompt.ts` sigue sin inferir `category`).~~ ✅ **HECHO — corregido 2026-07-22:** commit `07c9136` (20/07/2026, "inferir el dominio del objetivo dictado #10") lo agregó. `smartPrompt.ts` — `ProposedSmart` ahora tiene `category?: GoalCategory` (línea 59), el prompt pide explícitamente inferir el dominio del texto libre (línea 70: "infiere el dominio del objetivo desde el texto... si no es claro, usa personal"), `parseSmart` valida contra el enum case-insensitive y devuelve `undefined` si no aplica (líneas 136-138, sin pisar el dominio ya elegido en modo normal). Wired en `SmartWizard.tsx` (modo dictado: `setInferredCategory(s.category)` línea 168, aplicado al guardar línea 204). 4 tests en `smartPrompt.test.ts` (líneas 89-96) cubren enum válido/case-insensitive/inválido→undefined. Este ítem había quedado mal marcado como pendiente en 3 lugares del doc (bloques 07-20 y 07-21 de arriba) pese a que el commit es anterior a ambos pases — no fue detectado por error. ~~Narrative Intelligence (sin rastro en el código — no encontrado)~~ ✅ **corregido 2026-07-20:** SÍ existe — carpeta `src/app/api/self/` completa (rumbo/coherencia/arquetipo/retrato/premortem/espejo-*) + `src/lib/self/rumboPrompt.ts` — este pase 07-17 no lo encontró por error (ya lo documentaba `docs/STRATEGIC_ROADMAP.md` como CONSTRUIDO).
