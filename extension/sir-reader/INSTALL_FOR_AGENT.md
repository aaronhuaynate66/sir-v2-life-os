...
(Si el repo ya está en la PC: `git pull` y anda a `extension/sir-reader`.)
...
> No subas al repo este cambio (el token es secreto). Es solo local en esta PC.
...
- Si dice **"Falta el token"** → revisa el Paso 2 (`config.js`) y recarga la extensión (botón ↻ en `chrome://extensions`).
- Si el contador **no sube** aunque veas mensajes: en el popup, click **"Probar detección (en la pestaña activa)"** (estando en la pestaña de Teams/WhatsApp con un chat abierto). Te dice si encontró el hilo, el contenedor y cuántos mensajes extrajo, con una muestra y una pista del DOM. **Copia ESE reporte y pásalo** — con eso se ajustan los selectores exactos. (También puedes mirar la consola: F12 → Console → filtro `SIR Reader`.)
...
- Idempotente: reenviar mensajes no duplica (SIR deduplica por hash), así que no pasa nada si recargas la extensión.

## Qué le reportas al humano
